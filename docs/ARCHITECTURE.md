...
iOS 26+ `@Observable` macro eliminates `@Published` boilerplate and enables fine-grained dependency tracking. ViewModels and managers are marked `@Observable final class`.
...
| `BackgroundStore` | `@Observable` | Global + per-chat gradient/animation settings; `resolvedPreset(for:)` + `resolvedAnimation(for:)` |
...
| `ToolExecutorService` | `@Observable` | Executes `[TOOL: ...]` calls and returns tool outputs for chat rendering |
...
User taps Send in MessageInputView
  ...
  → AIPipeline.run(session:persona:config:..., memoryService:, toolExecutor:)
...
User creates post
  → SocialService.onMomentsPosted(total: userPosts)
  → MomentsOrchestrator.reactToUserPost()
      → 15–40s delay → 2–4 personas like/react
      → 30–90s delay → 1–2 personas comment (via AI)
User likes a post
  → MomentsStore.toggleLike(...) -> true when newly liked
  → SocialService.onMomentLiked()
```
...
**Background change:**
```
User picks style in BackgroundPickerView (per-chat or global)
  → BackgroundStore.setChat(sessionId:presetId:) / setGlobal(presetId:)
  → BackgroundStore.setChatAnimation(sessionId:animation:) / setGlobalAnimation(_:)
  → UserDefaults persisted
  → ChatView ZStack re-reads resolvedPreset(for: sessionId)
  → ChatView re-reads resolvedAnimation(for: sessionId)
  → if ThemeManager.animationIntensity == .none then animation is forced to .none
  → Animated background layer updates behind message list
```
...
Backup/export note: `AppBackup.storageData` stores raw serialized blobs from `StorageService.exportAll()` and is restored first during import.
...
iOS 26+ 的 `@Observable` 宏消除了 `@Published` 样板代码，并启用细粒度依赖追踪。所有 ViewModel 和 Manager 均标记为 `@Observable final class`。
...
| `BackgroundStore` | `@Observable` | 全局 + 每聊天渐变/动画配置；`resolvedPreset(for:)` + `resolvedAnimation(for:)` |
...
| `ToolExecutorService` | `@Observable` | 执行 `[TOOL: ...]` 调用并把工具结果返回给聊天 UI |
...
用户点击发送
  ...
  → AIPipeline.run(..., memoryService:, toolExecutor:)
      → 构建系统提示（性格 + 心情 + 亲密度）
...
用户发布动态
  → SocialService.onMomentsPosted(total: 用户动态数)
  → MomentsOrchestrator.reactToUserPost()
      → 等待15~40秒 → 2~4个角色点赞/表情
      → 等待30~90秒 → 1~2个角色评论（AI生成）
用户点赞动态
  → MomentsStore.toggleLike(...) 返回 true（新增点赞）
  → SocialService.onMomentLiked()
```
...
**更换聊天背景：**
```
用户在 BackgroundPickerView 选择样式（每聊天或全局）
  → BackgroundStore.setChat(sessionId:presetId:) / setGlobal(presetId:)
  → BackgroundStore.setChatAnimation(sessionId:animation:) / setGlobalAnimation(_:)
  → UserDefaults 持久化
  → ChatView ZStack 重新读取 resolvedPreset(for: sessionId)
  → ChatView 重新读取 resolvedAnimation(for: sessionId)
  → 若 ThemeManager.animationIntensity == .none，则动画强制为 .none
  → 消息列表后方动画背景层更新
```
...
| `chat-buddy:memories` | `MemoriesData` | 每角色记忆记录（`[String: [CharacterMemory]]`） |
| `chatSessions` | `[ChatSession]` | 全部聊天记录（无前缀） |

备份说明：`AppBackup.storageData` 会保存 `StorageService.exportAll()` 的原始序列化数据，并在导入时优先恢复。
