...
## 2. Capture or Request a Profile
...
Profiler, and Hangs/Hitches as relevant. Use available profiling tools to capture
the evidence. Request a trace or screenshots of the lanes and call tree only
when the required capture cannot be obtained with available access.
...
Re-run the same capture with available profiling tools and compare with baseline metrics. If the required device or capture access is unavailable, state that limitation and ask the user only for the missing capture.
