...
metadata: {"project_scoped":true,"os":["darwin","linux","win32"]}
---
...
Optional repository-specific notes live in `.agents/skill-state/github-actions/`.
Do not create them unless they will preserve useful, stable project context. See
`memory-template.md` for the baseline structure.

```text
.agents/skill-state/github-actions/
...
Repository-local notes in `.agents/skill-state/github-actions/` may include:
...
- repository operating notes under `.agents/skill-state/github-actions/`
